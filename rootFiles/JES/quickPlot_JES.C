void quickPlot_JES(){
  ...
  TFile *f1 = TFile::Open("JES_C0_pthat30.root");
  TFile *f2 = TFile::Open("JES_C0_pthat40.root");
  TFile *f3 = TFile::Open("JES_C0_pthat50.root");

  TH1D *h1, *h2, *h3;

  f1->GetObject("JES_result_b",h1);
  f2->GetObject("JES_result_b",h2);
  f3->GetObject("JES_result_b",h3);
  ...
  h1->SetStats(0);
  h1->SetTitle("");
  h1->GetYaxis()->SetTitle("#mu(#font[52]{p}_{T}^{recoJet} / #font[52]{p}_{T}^{genJet})");
  h1->Draw();
  h2->Draw("same");
  h3->Draw("same");
  TLegend *leg = new TLegend(0.3,0.55,0.4,0.75);
