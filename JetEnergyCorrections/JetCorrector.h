...
double JetCorrector::GetCorrection()
{
   ...
   if(PT < 0)
      //cout << "Flag in JetCorrector::GetCorrection() !";
      return -1;
   return PT / JetPT;
}
...
void SingleJetCorrector::Initialize(std::string FileName)
{
   ...
   while(in)
   {
      ...
      if(IsDefinition == true)
      {
         ...
         if( Parts.size() <= (long unsigned int) nvar + 1)
            continue;
         npar = atoi(Parts[nvar+1].c_str());
         if( Parts.size() <= (long unsigned int) nvar + 1 + npar + 1)
            continue;
      ...
      else
      {
         ...
         if( Parts.size() < (long unsigned int) nvar * 2 + npar * 2 + 1)
            continue;
...
double SingleJetCorrector::GetCorrection()
{
   ...
   for(int iE = 0; iE < N; iE++)
   {
      ...
      for(int i = 0; i < 3; i++)
      {
         if( Dependencies[iE].size() <= (long unsigned int) i)
            continue;
