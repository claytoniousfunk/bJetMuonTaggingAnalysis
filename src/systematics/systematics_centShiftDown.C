...
void systematics_centShiftDown(bool do_mu5 = 0,
			       bool do_mu7 = 0,
			       bool do_mu12 = 1){
  ...
  cout << endl;
  cout << "#######  SYSTEMATICS -- Centrality Shift UP  #########" << endl;
  cout << endl;

  TFile *file_b_noSmear, *file_b_smear;

  file_b_noSmear = TFile::Open("/home/clayton/Analysis/code/bJetMuonTaggingAnalysis/rootFiles/bPurityResults/latest/bPurityResults_DATA-1_mergeB-1_mergeC-1_pTrel-0.0to5.0_cMult-1.0_bMult-1.175_JERsmear-0_JEUShiftUp-0_JEUShiftDown-0_centShiftUp-0_centShiftDown-0.root");
  file_b_smear = TFile::Open("/home/clayton/Analysis/code/bJetMuonTaggingAnalysis/rootFiles/bPurityResults/latest/bPurityResults_DATA-1_mergeB-1_mergeC-1_pTrel-0.0to5.0_cMult-1.0_bMult-1.175_JERsmear-0_JEUShiftUp-0_JEUShiftDown-0_centShiftUp-0_centShiftDown-1.root");
  ...
  pp_canv_save_string = "/home/clayton/Analysis/code/bJetMuonTaggingAnalysis/figures/systematics/cent-shift-down/cent-shift-down.pdf";
  ...
  for(int i = 1; i < r_pp->GetSize() - 1; i++){
    err_pp_rms += (1./r_pp->GetBinError(i))*(1./r_pp->GetBinError(i))*TMath::Abs(1.0 - r_pp->GetBinContent(1))*TMath::Abs(1.0 - r_pp->GetBinContent(1));
    err_pp_weightSum += (1./r_pp->GetBinError(i))*(1./r_pp->GetBinError(i));
  }
  err_pp_rms = sqrt(err_pp_rms / err_pp_weightSum);
  ...
  la->DrawLatexNDC(0.22, 0.77, "Cent. Shift Down systematic");
  ...
  // la->DrawLatexNDC(0.82,0.29,Form("%3.2f %%",100*err_C2_J6));
  // la->SetTextColor(kBlue-4);
  // la->DrawLatexNDC(0.82,0.26,Form("%3.2f %%",100*err_C1_J6));  


  // la->SetTextSize(0.032);
  // la->SetTextColor(kBlue-4);
  // la->DrawLatexNDC(0.3,0.35,Form("%3.2f %%",100*sys_err_val_pp_J2));
  // la->SetTextColor(kRed-4);
  // la->DrawLatexNDC(0.3,0.31,Form("%3.2f %%",100*sys_err_val_pp_J3));
  // la->SetTextColor(kGreen+2);
  // la->DrawLatexNDC(0.3,0.27,Form("%3.2f %%",100*sys_err_val_pp_J4));
  // la->SetTextSize(0.036);
