...
void systematics_lowerBound(){
  ...
  // C4 ranges
  const int N_c_input_C4_J1 = 4;
  double c_input_C4_J1[N_c_input_C4_J1] = {0,0.1,0.2,0.3};

  const int N_c_input_C4_J2 = 4;
  double c_input_C4_J2[N_c_input_C4_J2] = {0,0.1,0.2,0.3};

  const int N_c_input_C4_J3 = 4;
  double c_input_C4_J3[N_c_input_C4_J3] = {0,0.1,0.2,0.3};
  
  const int N_c_input_C4_J4 = 4;
  double c_input_C4_J4[N_c_input_C4_J4] = {0,0.1,0.2,0.3};

  const int N_c_input_C4_J5 = 4;
  double c_input_C4_J5[N_c_input_C4_J5] = {0,0.1,0.2,0.3};

  const int N_c_input_C4_J6 = 4;
  double c_input_C4_J6[N_c_input_C4_J6] = {0,0.1,0.2,0.3};
  ...
  //   //    zeros_pp_J1[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  //   //    zeros_pp_J2[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  //   //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  //   //    zeros_pp_J3[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  //   //    zeros_pp_J4[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  //   //    zeros_pp_J5[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  //   //    zeros_pp_J6[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 1,0,0,0,0,  low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_pp_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  // pp_data_info_string = "#font[62]{CMS }#font[52]{#scale[0.8]{Preliminary}}   #font[42]{pp 5.02 TeV (301 pb^{-1})}";
  ...
  //   //    zeros_C4_J1[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  //   //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  //   //    zeros_C4_J2[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  //   //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  //   //    zeros_C4_J3[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  //   //    zeros_C4_J4[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  //   //    zeros_C4_J5[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  //   //    zeros_C4_J6[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C4_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  // mg_C4->GetXaxis()->SetTitle("Fit lower bound [GeV]");
  // mg_C4->SetTitle("");
  // mg_C4->Add(gr_C4_J1);  
  // mg_C4->Add(gr_C4_J2);
  // mg_C4->Add(gr_C4_J3);
  ...
  // mg_C4->Add(gr_C4_J5);
  // mg_C4->Add(gr_C4_J6);    
  // mg_C4->GetXaxis()->SetLimits(-0.05,0.55);
  // mg_C4->GetYaxis()->SetRangeUser(0.75,1.25);
  // mg_C4->Draw("AP");
  // TLegend *leg_mg_C4 = new TLegend(0.55,0.65,0.88,0.85);
  ...
  // C4_data_info_string = "#font[62]{CMS }#font[52]{#scale[0.8]{Preliminary}} #font[42]{PbPb 50-90% 5.02 TeV (1689 #mub^{-1})}";
  ...
  // C4_canv_save_string = "/home/clayton/Analysis/code/bJetMuonTaggingAnalysis/figures/systematics/lower-bound/C4.pdf";
  // la->SetTextSize(0.032);
  // la->SetTextColor(kBlack);
  // la->DrawLatexNDC(0.2,0.43,Form("%3.2f %%",100*sys_err_val_C4_J1));
  // la->SetTextColor(kBlue-4);
  // la->DrawLatexNDC(0.2,0.39,Form("%3.2f %%",100*sys_err_val_C4_J2));
  // la->SetTextColor(kRed-4);
  // la->DrawLatexNDC(0.2,0.35,Form("%3.2f %%",100*sys_err_val_C4_J3));
  // la->SetTextColor(kGreen+2);
  // la->DrawLatexNDC(0.2,0.31,Form("%3.2f %%",100*sys_err_val_C4_J4));
  // la->SetTextColor(kMagenta-3);
  // la->DrawLatexNDC(0.2,0.27,Form("%3.2f %%",100*sys_err_val_C4_J5));
  // la->SetTextColor(kOrange);
  // la->DrawLatexNDC(0.2,0.23,Form("%3.2f %%",100*sys_err_val_C4_J6));
  // la->SetTextSize(0.036);
  // la->SetTextColor(kBlack);
 ...
 ////// *************************************************************************************************************** ///////////////
  ...
  //   //    zeros_C3_J1[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  //   //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  //   //    zeros_C3_J2[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  //   //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  //   //    zeros_C3_J3[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  //   //    zeros_C3_J4[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  //   //    zeros_C3_J5[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  //   //    zeros_C3_J6[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C3_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  // mg_C3->Draw("AP");
  // TLegend *leg_mg_C3 = new TLegend(0.55,0.65,0.88,0.85);
  ...
  // C3_data_info_string = "#font[62]{CMS }#font[52]{#scale[0.8]{Preliminary}} #font[42]{PbPb 30-50% 5.02 TeV (1689 #mub^{-1})}";
  ...
  //   //    zeros_C2_J1[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  //   //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  //   //    zeros_C2_J2[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  //   //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  //   //    zeros_C2_J3[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  //   //    zeros_C2_J4[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  //   //    zeros_C2_J5[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  //   //    zeros_C2_J6[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C2_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  // mg_C2->Draw("AP");
  // TLegend *leg_mg_C2 = new TLegend(0.55,0.65,0.88,0.85);
  ...
  // C2_data_info_string = "#font[62]{CMS }#font[52]{#scale[0.8]{Preliminary}} #font[42]{PbPb 10-30% 5.02 TeV (1689 #mub^{-1})}";
  ...
  for(int i = 0; i < N_c_input_C1_J1; i++){
    ...
    //    zeros_C1_J1[i] = 0.0;
    double result_i  = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
    double err_i = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
    double chi2ndof_i = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J1[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
    //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  for(int i = 0; i < N_c_input_C1_J2; i++){
    ...
    //    zeros_C1_J2[i] = 0.0;
    double result_i  = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
    double err_i = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
    double chi2ndof_i = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J2[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
    //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  for(int i = 0; i < N_c_input_C1_J3; i++){
    ...
    //    zeros_C1_J3[i] = 0.0;
    double result_i  = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
    double err_i = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
    double chi2ndof_i = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J3[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  for(int i = 0; i < N_c_input_C1_J4; i++){
    ...
    //    zeros_C1_J4[i] = 0.0;
    double result_i  = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
    double err_i = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
    double chi2ndof_i = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J4[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  for(int i = 0; i < N_c_input_C1_J5; i++){
    ...
    //    zeros_C1_J5[i] = 0.0;
    double result_i  = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
    double err_i = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
    double chi2ndof_i = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J5[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  for(int i = 0; i < N_c_input_C1_J6; i++){
    ...
    //    zeros_C1_J6[i] = 0.0;
    double result_i  = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 1);
    double err_i = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 2);
    double chi2ndof_i = templateFitter(do_data, 0,1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  c_input_C1_J6[i],pTrel_fit_high, c_multiplier , bGS_multiplier, 3);
  ...
  mg_C1->Draw("AP");
  TLegend *leg_mg_C1 = new TLegend(0.55,0.65,0.88,0.85);
  ...
  C1_data_info_string = "#font[62]{CMS }#font[52]{#scale[0.8]{Preliminary}} #font[42]{PbPb 0-10% 5.02 TeV (1689 #mub^{-1})}";
