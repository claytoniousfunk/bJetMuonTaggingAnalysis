...
void systematics_JERsmear(bool do_mu5 = 0,
  ...
  file_b_noSmear = TFile::Open("/home/clayton/Analysis/code/bJetMuonTaggingAnalysis/rootFiles/bPurityResults/latest/bPurityResults_DATA-1_mergeB-1_mergeC-1_pTrel-0.0to5.0_cMult-1.0_bMult-1.175_JERsmear-0_JEUShiftUp-0_JEUShiftDown-0_bJetEnergyShift-0.root");
  file_b_smear = TFile::Open("/home/clayton/Analysis/code/bJetMuonTaggingAnalysis/rootFiles/bPurityResults/latest/bPurityResults_DATA-1_mergeB-1_mergeC-1_pTrel-0.0to5.0_cMult-1.0_bMult-1.175_JERsmear-1_JEUShiftUp-0_JEUShiftDown-0_bJetEnergyShift-0.root");
 
  TH1D *b_result_pp_noSmear, *b_result_pp_smear;
  TH1D *b_result_C4_noSmear, *b_result_C4_smear;
  ...
  file_b_noSmear->GetObject("bFracResults_C4",b_result_C4_noSmear);
  ...
  file_b_smear->GetObject("bFracResults_C4",b_result_C4_smear);
  ...
  TH1D *r_pp, *r_C2, *r_C1, *r_C3, *r_C4;

  r_pp = (TH1D*) b_result_pp_noSmear->Clone("r_pp");
  r_pp->Divide(b_result_pp_noSmear,b_result_pp_smear,1,1,"");

  r_C4 = (TH1D*) b_result_C4_noSmear->Clone("r_C4");
  r_C4->Divide(b_result_C4_noSmear,b_result_C4_smear,1,1,"");
  ...
  TLegend *leg_r_pp = new TLegend(0.6,0.70,0.88,0.88);
  leg_r_pp->SetBorderSize(0);
  leg_r_pp->SetTextSize(0.031);
  leg_r_pp->AddEntry(r_pp,"pp","p");
  leg_r_pp->AddEntry(r_C4,"PbPb 50-80%","p");
  leg_r_pp->AddEntry(r_C3,"PbPb 30-50%","p");
  ...
  pp_canv_save_string = "/home/clayton/Analysis/code/bJetMuonTaggingAnalysis/figures/systematics/JER-smear/JER-smear.pdf";
  ...
  // double err_pp_rms = sqrt((err_pp_J1*err_pp_J1 + err_pp_J2*err_pp_J2 + err_pp_J3*err_pp_J3 + err_pp_J4*err_pp_J4 + err_pp_J5*err_pp_J5 + err_pp_J6*err_pp_J6)/6);
  double err_pp_rms = 0.;
  double err_pp_sum = 0.;
  double err_pp_weightSum = 0.;
  for(int i = 1; i < r_pp->GetSize() - 1; i++){
    err_pp_sum += (1./r_pp->GetBinError(i))*(1./r_pp->GetBinError(i))*TMath::Abs(1.0 - r_pp->GetBinContent(1))*TMath::Abs(1.0 - r_pp->GetBinContent(1));
    err_pp_weightSum += (1./r_pp->GetBinError(i))*(1./r_pp->GetBinError(i));
    std::cout << "r_pp->GetBinError(" << i << ") = " << r_pp->GetBinError(i) << "\n";
  }
  err_pp_rms = sqrt(err_pp_sum / err_pp_weightSum);
  std::cout << "err_pp_rms = " << err_pp_rms << "\n";

  // double err_C4_J1 = TMath::Abs(1.0 - r_C4->GetBinContent(1));
  // double err_C4_J2 = TMath::Abs(1.0 - r_C4->GetBinContent(2));
  // double err_C4_J3 = TMath::Abs(1.0 - r_C4->GetBinContent(3));
  // double err_C4_J4 = TMath::Abs(1.0 - r_C4->GetBinContent(4));
  // double err_C4_J5 = TMath::Abs(1.0 - r_C4->GetBinContent(5));
  // double err_C4_J6 = TMath::Abs(1.0 - r_C4->GetBinContent(6));
  // double err_C4_rms = sqrt((err_C4_J1*err_C4_J1 + err_C4_J2*err_C4_J2 + err_C4_J3*err_C4_J3 + err_C4_J4*err_C4_J4 + err_C4_J5*err_C4_J5 + err_C4_J6*err_C4_J6)/6);
  double err_C4_rms = 0.;
  double err_C4_weightSum = 0.;
  for(int i = 1; i < r_C4->GetSize() - 1; i++){
    err_C4_rms += (1./r_C4->GetBinError(i))*(1./r_C4->GetBinError(i))*TMath::Abs(1.0 - r_C4->GetBinContent(1))*TMath::Abs(1.0 - r_C4->GetBinContent(1));
    err_C4_weightSum += (1./r_C4->GetBinError(i))*(1./r_C4->GetBinError(i));
  }
  err_C4_rms = sqrt(err_C4_rms / err_C4_weightSum);
  std::cout << "err_C4_rms = " << err_C4_rms << "\n";
  ...
  // double err_C3_rms = sqrt((err_C3_J1*err_C3_J1 + err_C3_J2*err_C3_J2 + err_C3_J3*err_C3_J3 + err_C3_J4*err_C3_J4 + err_C3_J5*err_C3_J5 + err_C3_J6*err_C3_J6)/6);
  double err_C3_rms = 0.;
  double err_C3_weightSum = 0.;
  for(int i = 1; i < r_C3->GetSize() - 1; i++){
    err_C3_rms += (1./r_C3->GetBinError(i))*(1./r_C3->GetBinError(i))*TMath::Abs(1.0 - r_C3->GetBinContent(1))*TMath::Abs(1.0 - r_C3->GetBinContent(1));
    err_C3_weightSum += (1./r_C3->GetBinError(i))*(1./r_C3->GetBinError(i));
  }
  err_C3_rms = sqrt(err_C3_rms / err_C3_weightSum);
  std::cout << "err_C3_rms = " << err_C3_rms << "\n";
  ...
  // double err_C2_rms = sqrt((err_C2_J1*err_C2_J1 + err_C2_J2*err_C2_J2 + err_C2_J3*err_C2_J3 + err_C2_J4*err_C2_J4 + err_C2_J5*err_C2_J5 + err_C2_J6*err_C2_J6)/6);
  double err_C2_rms = 0.;
  double err_C2_weightSum = 0.;
  for(int i = 1; i < r_C2->GetSize() - 1; i++){
    err_C2_rms += (1./r_C2->GetBinError(i))*(1./r_C2->GetBinError(i))*TMath::Abs(1.0 - r_C2->GetBinContent(1))*TMath::Abs(1.0 - r_C2->GetBinContent(1));
    err_C2_weightSum += (1./r_C2->GetBinError(i))*(1./r_C2->GetBinError(i));
  }
  err_C2_rms = sqrt(err_C2_rms / err_C2_weightSum);
  std::cout << "err_C2_rms = " << err_C2_rms << "\n";
  ...
  // double err_C1_rms = sqrt((err_C1_J1*err_C1_J1 + err_C1_J2*err_C1_J2 + err_C1_J3*err_C1_J3 + err_C1_J4*err_C1_J4 + err_C1_J5*err_C1_J5 + err_C1_J6*err_C1_J6)/6);
  double err_C1_rms = 0.;
  double err_C1_weightSum = 0.;
  for(int i = 1; i < r_C1->GetSize() - 1; i++){
    err_C1_rms += (1./r_C1->GetBinError(i))*(1./r_C1->GetBinError(i))*TMath::Abs(1.0 - r_C1->GetBinContent(1))*TMath::Abs(1.0 - r_C1->GetBinContent(1));
    err_C1_weightSum += (1./r_C1->GetBinError(i))*(1./r_C1->GetBinError(i));
  }
  err_C1_rms = sqrt(err_C1_rms / err_C1_weightSum);
  std::cout << "err_C1_rms = " << err_C1_rms << "\n";



  la->SetTextSize(0.033);
  la->DrawLatexNDC(0.22, 0.77, "JER smear systematic");
  
  la->SetTextSize(0.029);
  la->SetTextColor(kGreen+2);
  la->DrawLatexNDC(0.22,0.35,Form("%3.2f %%",100*err_pp_rms));
  la->SetTextColor(kOrange-2);
  la->DrawLatexNDC(0.22,0.32,Form("%3.2f %%",100*err_C4_rms));
  la->SetTextColor(kRed-4);
  la->DrawLatexNDC(0.22,0.29,Form("%3.2f %%",100*err_C3_rms));
  la->SetTextColor(kBlue-4);
  la->DrawLatexNDC(0.22,0.26,Form("%3.2f %%",100*err_C2_rms));
  la->SetTextColor(kMagenta-9);
  la->DrawLatexNDC(0.22,0.23,Form("%3.2f %%",100*err_C1_rms));
