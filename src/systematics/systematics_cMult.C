...
void systematics_cMult(){
  ...
  for(int i = 0; i < N_c_input_pp_J1; i++){
    ...
    //    zeros_pp_J1[i] = 0.0;
    double result_i  = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J1[i] , bGS_multiplier, 1);
    double err_i = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J1[i] , bGS_multiplier, 2);
    double chi2ndof_i = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J1[i] , bGS_multiplier, 3);
  ...
  for(int i = 0; i < N_c_input_pp_J2; i++){
    ...
    //    zeros_pp_J2[i] = 0.0;
    double result_i  = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J2[i] , bGS_multiplier, 1);
    double err_i = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J2[i] , bGS_multiplier, 2);
    double chi2ndof_i = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J2[i] , bGS_multiplier, 3);
    //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  for(int i = 0; i < N_c_input_pp_J3; i++){
    ...
    //    zeros_pp_J3[i] = 0.0;
    double result_i  = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J3[i] , bGS_multiplier, 1);
    double err_i = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J3[i] , bGS_multiplier, 2);
    double chi2ndof_i = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J3[i] , bGS_multiplier, 3);
  ...
  for(int i = 0; i < N_c_input_pp_J4; i++){
    ...
    //    zeros_pp_J4[i] = 0.0;
    double result_i  = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J4[i] , bGS_multiplier, 1);
    double err_i = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J4[i] , bGS_multiplier, 2);
    double chi2ndof_i = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J4[i] , bGS_multiplier, 3);
  ...
  for(int i = 0; i < N_c_input_pp_J5; i++){
    ...
    //    zeros_pp_J5[i] = 0.0;
    double result_i  = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J5[i] , bGS_multiplier, 1);
    double err_i = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J5[i] , bGS_multiplier, 2);
    double chi2ndof_i = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J5[i] , bGS_multiplier, 3);
  ...
  for(int i = 0; i < N_c_input_pp_J6; i++){
    ...
    //    zeros_pp_J6[i] = 0.0;
    double result_i  = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J6[i] , bGS_multiplier, 1);
    double err_i = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J6[i] , bGS_multiplier, 2);
    double chi2ndof_i = templateFitter(do_data, 1,0,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_pp_J6[i] , bGS_multiplier, 3);
  ...
  //   //    zeros_C3_J1[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J1[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J1[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J1[i] , bGS_multiplier, 3);
  //   //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  //   //    zeros_C3_J2[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J2[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J2[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J2[i] , bGS_multiplier, 3);
  //   //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  //   //    zeros_C3_J3[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J3[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J3[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J3[i] , bGS_multiplier, 3);
  ...
  //   //    zeros_C3_J4[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J4[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J4[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J4[i] , bGS_multiplier, 3);
  ...
  //   //    zeros_C3_J5[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J5[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J5[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J5[i] , bGS_multiplier, 3);
  ...
  //   //    zeros_C3_J6[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J6[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J6[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,0,1, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C3_J6[i] , bGS_multiplier, 3);
  ...
  // C3_data_info_string = "#font[62]{CMS }#font[52]{#scale[0.8]{Premilinary}}   #font[42]{PbPb 30-80% 5.02 TeV)}";
  ...
  //   //    zeros_C2_J1[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J1[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J1[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J1[i] , bGS_multiplier, 3);
  //   //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  //   //    zeros_C2_J2[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J2[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J2[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J2[i] , bGS_multiplier, 3);
  //   //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  //   //    zeros_C2_J3[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J3[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J3[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J3[i] , bGS_multiplier, 3);
  ...
  //   //    zeros_C2_J4[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J4[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J4[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J4[i] , bGS_multiplier, 3);
  ...
  //   //    zeros_C2_J5[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J5[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J5[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J5[i] , bGS_multiplier, 3);
  ...
  //   //    zeros_C2_J6[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J6[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J6[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,0,1,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C2_J6[i] , bGS_multiplier, 3);
  ...
  //   //    zeros_C1_J1[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J1[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J1[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J1[i] , bGS_multiplier, 3);
  //   //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  //   //    zeros_C1_J2[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J2[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J2[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J2[i] , bGS_multiplier, 3);
  //   //cout << "CHI2 / NDOF ===== " << chi2ndof_i << endl;
  ...
  //   //    zeros_C1_J3[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J3[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J3[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J3[i] , bGS_multiplier, 3);
  ...
  //   //    zeros_C1_J4[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J4[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J4[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J4[i] , bGS_multiplier, 3);
  ...
  //   //    zeros_C1_J5[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J5[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J5[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J5[i] , bGS_multiplier, 3);
  ...
  //   //    zeros_C1_J6[i] = 0.0;
  //   double result_i  = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J6[i] , bGS_multiplier, 1);
  //   double err_i = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J6[i] , bGS_multiplier, 2);
  //   double chi2ndof_i = templateFitter(do_data, 0,1,0,0, low_jetPt,high_jetPt, do_mergeB,do_mergeC,  do_2tempFit,do_3tempFit,  pTrel_fit_low,pTrel_fit_high, c_input_C1_J6[i] , bGS_multiplier, 3);
