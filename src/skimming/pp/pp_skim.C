...
void pp_skim(int group = 0,
  ...
  output_file_base += Form("output/pp_skim_slim_output_%i",startgroup) ;
  ...
  my_filter_tree->Branch("pBeamScrapingFilter",&pBeamScrapingFilter);
  ...
  while(instr>>filename && ifile<endfile){
    ...
    filter_tree->SetBranchAddress("pBeamScrapingFilter",t_pBeamScrapingFilter);
    ...
    for(int evi = 0; evi < n_evt; evi++){
      ...
      pBeamScrapingFilter = t_pBeamScrapingFilter[0];

      // --- event-filter cuts
      if(pPAprimaryVertexFilter         == 0 || 
         HBHENoiseFilterResultRun2Loose == 0 || 
	 pBeamScrapingFilter            == 0
	 ) continue;
