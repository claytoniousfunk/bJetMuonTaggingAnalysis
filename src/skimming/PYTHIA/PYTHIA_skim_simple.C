void PYTHIA_skim_simple(int group = 1){

  string in_file_name;
  string output_file_base = "";
  in_file_name = "../../../fileNames/fileNames_PYTHIA_DiJet_withGS_withNeutrinos.txt";
  output_file_base += Form("/eos/cms/store/group/phys_heavyions/cbennett/output_skim_PYTHIA_DiJet_withGS_withNeutrinos/PYTHIA_DiJet_skim_output_%i",group);
  ...
  int endfile = 987; // PYTHIA_withGS_withNeutrinos forest
  ...
  while(instr>>filename && ifile<endfile){
    ...
    old_jet_tree->SetBranchStatus("mueta",1);
