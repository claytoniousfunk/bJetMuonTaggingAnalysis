...
double fitfxn(double *x, double *par){
  ...
  double fitval = par[0] + par[1]*pt + par[2]*pt*pt + par[3]/pt + par[4]/(pt*pt);
  return fitval;
}
...
void RecoRefCompare_neutrinoPt(bool ispp = 1,
			       bool isC2 = 0,
			       bool isC1 = 0){
  ...
  if(ispp) f1 = TFile::Open("/home/clayton/Analysis/code/bJetMuonTaggingAnalysis/rootFiles/scanningOutput/PYTHIA/official/PYTHIA_mu12_response_withReco_muTaggedJets_moreFlavors_withNeutrinos.root");
  else f1 = TFile::Open("~/Analysis/code/bJetMuonTaggingAnalysis/rootFiles/scanningOutput/PYTHIAHYDJET/PYTHIAHYDJET_DiJet_response_pthat15.root"); 
  ...
  if(ispp){
    ...
    // f1->GetObject("h_matchedRecoJetPtOverGenJetPt_genJetPt_bJets",h7);

    // f1->GetObject("h_matchedNeutrinoPtOverGenJetPt_genJetPt_allJets",h1);
    // f1->GetObject("h_matchedNeutrinoPtOverGenJetPt_genJetPt_udJets",h2);
    // f1->GetObject("h_matchedNeutrinoPtOverGenJetPt_genJetPt_sJets",h3);
    // f1->GetObject("h_matchedNeutrinoPtOverGenJetPt_genJetPt_gJets",h4);
    // f1->GetObject("h_matchedNeutrinoPtOverGenJetPt_genJetPt_cJets",h5);
    // f1->GetObject("h_matchedNeutrinoPtOverGenJetPt_genJetPt_bJets",h6);
    // f1->GetObject("h_matchedNeutrinoPtOverGenJetPt_genJetPt_bJets",h7);

    f1->GetObject("h_matchedNeutrinoPtOverRecoJetPt_recoJetPt_allJets",h1);
    f1->GetObject("h_matchedNeutrinoPtOverRecoJetPt_recoJetPt_udJets",h2);
    f1->GetObject("h_matchedNeutrinoPtOverRecoJetPt_recoJetPt_sJets",h3);
    f1->GetObject("h_matchedNeutrinoPtOverRecoJetPt_recoJetPt_gJets",h4);
    f1->GetObject("h_matchedNeutrinoPtOverRecoJetPt_recoJetPt_cJets",h5);
    f1->GetObject("h_matchedNeutrinoPtOverRecoJetPt_recoJetPt_bJets",h6);
    f1->GetObject("h_matchedNeutrinoPtOverRecoJetPt_recoJetPt_bJets",h7);
  ...
  const int Nbins = 24;
  double pt_axis[Nbins] = {40,50,60,70,80,90,100,110,120,130,140,150,160,170,180,190,200,210,220,240,260,300,350,400};

  // const int Nbins = 3;
  // double pt_axis[Nbins] = {200,210,220};

  // const int Nbins = 2;
  // double pt_axis[Nbins] = {50,60};
  ...
  TF1 *fit_b = new TF1("fit_b",fitfxn,0,500,5);
  ...
  TF1 *fxn1 = new TF1("fxn1",fitfxn,0,500,2);
  TF1 *fxn2 = new TF1("fxn2",fitfxn,0,500,2);
  TF1 *fxn3 = new TF1("fxn3",fitfxn,0,500,2);
  TF1 *fxn4 = new TF1("fxn4",fitfxn,0,500,2);
  TF1 *fxn5 = new TF1("fxn5",fitfxn,0,500,2);
  TF1 *fxn6 = new TF1("fxn6",fitfxn,0,500,2);
  TF1 *fxn7 = new TF1("fxn7",fitfxn,0,500,2);
  ...
  for(int i = 0; i < Nbins-1; i++){
    ...
    double muParLimLow = 0.0;
    double muParLimHigh = 1.0;
    ...
    sig3 = fxn3->GetParameter(2);    fxn5->SetParameter(1,maxBinVal_5);
    ...
    h7_y->GetXaxis()->SetTitle("p_{T}^{#nu} / p_{T}^{recoJet}");
    h7_y->GetYaxis()->SetTitle("Weighted entries");
    h7_y->GetXaxis()->SetRangeUser(0,1.2);
    h7_y->Draw();
    fxn7->SetLineColor(kRed);
    fxn7->Draw("same");
    h7_y->SetTitle(Form("%3.0f < p_{T}^{recoJet} < %3.0f, bJets",pt_axis[i],pt_axis[i+1]));
    ctmp->SaveAs(Form("fits/bJets/bJet_fit_%3.0f_%3.0f.pdf",pt_axis[i],pt_axis[i+1]));
    ...
    // fxn1->Draw("same");
    // h1_y->SetTitle(Form("%3.0f < p_{T}^{genJet} < %3.0f, allJets",pt_axis[i],pt_axis[i+1]));
  ...
  mg->GetYaxis()->SetRangeUser(0.0,1.32);
  mg->GetXaxis()->SetRangeUser(0,500);
  mg->GetYaxis()->SetTitle("#mu (#font[52]{p}_{T}^{#nu} / #font[52]{p}_{T}^{recoJet})");
  ...
  TLegend *l = new TLegend(0.7,0.6,0.89,0.88);
  l->SetBorderSize(0);
  l->SetTextFont(42);
  l->SetTextSize(0.04);
  l->SetFillStyle(0);
  ...
  // l->AddEntry(gr7,"#font[52]{b} jets, no neutrino","p");
  ...
  for(int i = 0; i < Nbins; i++){
    ...
    double bin_i_b = gr6->GetPointY(i);
    double err_i_b = gr6->GetErrorY(i);
  ...
  canv_fit_test->cd();
  h_fit_b->SetTitle("Fits to <#it{p}_{T}^{#nu} / #it{p}_{T}^{recoJet}>");
  h_fit_b->GetXaxis()->SetTitle("#it{p}_{T}^{recoJet} [GeV]");
  h_fit_b->GetYaxis()->SetTitle("<#it{p}_{T}^{#nu} / #it{p}_{T}^{recoJet}>");
  h_fit_b->SetStats(0);
  h_fit_b->Draw();
  //h_fit_i->Draw("same");
  // h_fit_i->Fit("pol3","W M");
  //h_fit_b->Fit("pol3","W M");
  double fitLow = 50.0;
  double fitHigh = 500.0;
  TString fitOptionString = "W M";
  //TString fitOptionString = "";
  
  //fit_b->SetParameter(0,720);
  //fit_b->SetParameter(1,-0.285);
  
  
  h_fit_b->Fit(fit_b,fitOptionString,"",fitLow,fitHigh);
  //h_fit_b->Fit(fxn1,fitOptionString,"",fitLow,fitHigh);
  ...
  t1->DrawLatexNDC(x_t1,y_t3,"#mu-tagged jets, |#eta^{jet}| < 1.6");
  ...
  // TF1 *func = new TF1("func",JES_fit,50.,300.,3);
  //Gr1->Fit(func,"M R WL","",50.0,300.0);
  ...
  // if(ispp){
  //   c1->SaveAs("/home/clayton/Analysis/code/bJet_analysis/JES/pp_JES.pdf");
  //   c2->SaveAs("/home/clayton/Analysis/code/bJet_analysis/JER/pp_JER.pdf");
  // }
  // else if(isC2){
  //   c1->SaveAs("/home/clayton/Analysis/code/bJet_analysis/JES/PbPb_30_90_JES.pdf");
  //   c2->SaveAs("/home/clayton/Analysis/code/bJet_analysis/JER/PbPb_30_90_JER.pdf");
  // }
  // else if(isC1){
  //   c1->SaveAs("/home/clayton/Analysis/code/bJet_analysis/JES/PbPb_0_30_JES.pdf");
  //   c2->SaveAs("/home/clayton/Analysis/code/bJet_analysis/JER/PbPb_0_30_JER.pdf");
  // }
  // else{};
