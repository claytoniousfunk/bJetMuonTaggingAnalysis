...
#include "plotJetTrkMaxDR_jetAxisSmearingEffect_tune.h"
...
TString getJetPtString(int j){
  TString result = "";
  if(j == 0) result = "80 < #it{p}_{T}^{jet} (GeV) < 500";
  else if(j == 1) result = "80 < #it{p}_{T}^{jet} (GeV) < 100";
...
void plotJetTrkMaxDR_jetAxisSmearingEffect_tune(int j = 1){
  ...
  if(!file_pp_mc_raw || !file_pp_mc_smeared_1){
    std::cout << "error finding file! exiting\n";
    return;
  }

  file_pp_mc_raw->GetObject(Form("h_jetTrkMaxDR_J%i",j),h_dR_pp_mc_raw);
  file_pp_mc_smeared_1->GetObject(Form("h_jetTrkMaxDR_J%i",j),h_dR_pp_mc_smeared);
  ...
  r_pp->Divide(h_dR_pp_data,h_dR_pp_mc_smeared,1,1,"");
  r_C5->Divide(h_dR_C5_mc_smeared,h_dR_C5_mc_raw,1,1,"");
  r_C4->Divide(h_dR_C4_mc_smeared,h_dR_C4_mc_raw,1,1,"");
  r_C3->Divide(h_dR_C3_mc_smeared,h_dR_C3_mc_raw,1,1,"");
  r_C2->Divide(h_dR_C2_mc_smeared,h_dR_C2_mc_raw,1,1,"");
  r_C1->Divide(h_dR_C1_mc_smeared,h_dR_C1_mc_raw,1,1,"");
  ...
  pad_pp_2->Draw();
  pad_pp_1->cd();
  h_dR_pp_mc_smeared->GetYaxis()->SetTitleSize(0.6*subTitleSize);
  h_dR_pp_mc_smeared->GetYaxis()->SetLabelSize(0.6*subLabelSize);
  h_dR_pp_mc_smeared->GetYaxis()->SetTitleOffset(0.9);
  h_dR_pp_mc_smeared->GetYaxis()->SetTitle("1/#it{N} d#it{N}/d#DeltaR");
  h_dR_pp_mc_smeared->GetYaxis()->SetRangeUser(0,0.15);
  h_dR_pp_mc_smeared->Draw();
  ...
  for(int i = 1; i < 120; i++){
    ...
    observed_i = r_pp->GetBinContent(i);
    weight_i = r_pp->GetBinError(i);
    cout << "observed(" << i << ") = " << observed_i << endl;
    //chi2prime += (observed_i - expected_i)*(observed_i - expected_i) / (expected_i * expected_i);
    if(weight_i > 0) chi2prime += (observed_i - expected_i)*(observed_i - expected_i) / (weight_i * weight_i);
    
  }

  chi2primeOverNDOF = chi2prime / (1. * (r_pp->GetNbinsX()-1));
  ...
  pad_pp_1->cd();
  la->SetTextSize(0.06);
  la->DrawLatexNDC(0.6,0.2,Form("#chi^{2}/ndof = %1.2f",chi2primeOverNDOF));

  canv_pp->SaveAs(Form("./figures/jetAxisSmearingEffectDR_tune/pp_%i.pdf",j));
  ...
  li->SetLineStyle(7);
  li->DrawLine(0,1,0.3,1);
  canv_C5->SaveAs(Form("./figures/jetAxisSmearingEffectDR_tune/C5_%i.pdf",j));
  ...
  li->SetLineStyle(7);
  li->DrawLine(0,1,0.3,1);
  canv_C4->SaveAs(Form("./figures/jetAxisSmearingEffectDR_tune/C4_%i.pdf",j));
  ...
  li->SetLineStyle(7);
  li->DrawLine(0,1,0.3,1);
  canv_C3->SaveAs(Form("./figures/jetAxisSmearingEffectDR_tune/C3_%i.pdf",j));
  ...
  li->SetLineStyle(7);
  li->DrawLine(0,1,0.3,1);
  canv_C2->SaveAs(Form("./figures/jetAxisSmearingEffectDR_tune/C2_%i.pdf",j));
  ...
  la->DrawLatexNDC(0.4,0.7,hadronPtString);
  pad_C1_2->cd();
  r_C1->GetYaxis()->SetTitle("Smeared / Data  ");
  ...
  li->SetLineStyle(7);
  li->DrawLine(0,1,0.3,1);
  canv_C1->SaveAs(Form("./figures/jetAxisSmearingEffectDR_tune/C1_%i.pdf",j));
