...
void plotJetPt_bJets(bool isPP = 1, bool isC2 = 0, bool isC1 = 0, bool isSingleMuon = 1, bool isMinBias = 0, bool isMu7 = 0, bool isMu12 = 1){
   ...
   if(isC2 || isC1){
	 ...
	 f2 = TFile::Open("/home/clayton/Analysis/code/bJetMuonTaggingAnalysis/rootFiles/scanningOutput/PYTHIAHYDJET/obsidian/fineCentBins/PYTHIAHYDJET_DiJet_withGS_scan_mu12_tight_pTmu-14_pThat-45_hiHFcut_removeHYDJETjet_jetTrkMaxFilter_vzReweight_hiBinReweight_newJetBins_fineCentBins.root");
       }
       else{};
     }
     else if(isMinBias){
       if(isMu12){
	 f1 = TFile::Open(goldenFile_PbPb_MinBias_mu12_newJetBins);
	 // f2 = TFile::Open();
       }
       else{};
     }
   ...
   TH2D *H1, *H2;
   TH1D *h1, *h1_add, *h1_add2, *h2, *h2_add, *h3, *h3_add;
   TH1D *h2_add2;


   if(isPP){
     if(isSingleMuon){
       f1->GetObject("h_inclRecoJetPt_inclRecoMuonTag_triggerOn_flavor",H1);
   ...
   if(isC1){
     if(isSingleMuon){
       f1->GetObject("h_inclRecoJetPt_inclRecoMuonTag_triggerOn_C1",h1);
       //f2->GetObject("h_inclGenJetPt_inclRecoMuonTag_triggerOn_flavor_C1",H1);
       //f2->GetObject("h_inclRecoJetPt_inclRecoMuonTag_triggerOn_flavor_C1",H2);
       f2->GetObject("h_inclRecoJetPt_inclRecoMuonTag_triggerOn_flavor_C1",H2);
     }
     else if(isMinBias){
   ...
   h1 = (TH1D*) H1->ProjectionX("h1",binFinder->FindBin(-5+smallShift),binFinder->FindBin(-4-smallShift));
   h1_add = (TH1D*) H1->ProjectionX("h1_add",binFinder->FindBin(5+smallShift),binFinder->FindBin(6-smallShift));
   h1_add2 = (TH1D*) H1->ProjectionX("h1_add2",binFinder->FindBin(17+smallShift),binFinder->FindBin(18-smallShift));
   ...
   double N_h0_b = h1->Integral(h1->GetXaxis()->FindBin(80),h1->GetXaxis()->FindBin(500));
   double N_h0_bGS = h2->Integral(h1->GetXaxis()->FindBin(80),h1->GetXaxis()->FindBin(500));
   ...
   //h2->Scale(((f_bGS_truth + GS_enhancement_shift)/N_h0_bGS)*(N_h0_b));
   h2->Scale(N_h0_b*(f_bGS_truth + GS_enhancement_shift) / (N_h0_bGS*(1-(f_bGS_truth + GS_enhancement_shift))));
   ...
   h1->Add(h2);
   double f_bGS_post = h2->Integral(h2->GetXaxis()->FindBin(80),h2->GetXaxis()->FindBin(500)) / h1->Integral(h1->GetXaxis()->FindBin(80),h1->GetXaxis()->FindBin(500));
   std::cout << "f_bGS_pre = " << f_bGS_truth << "\n";
   std::cout << "f_bGS_post = " << f_bGS_post << "\n";
   
   //h1->Scale(f_b_truth / h1->Integral());
   //h2->Scale(GS_enhancement_factor * f_bGS_truth / h2->Integral());
   //h2->Scale(bGS_multiplier * f_bGS_truth / h2->Integral());
   ...
   h1r->GetYaxis()->SetTitle("d#font[52]{N}^{jet}/d#font[52]{p}_{T}^{jet} [GeV^{-1}]");
   ...
   if(isPP){
     leg->AddEntry(h1r,"incl. #it{b}-jets","p");
     leg->AddEntry(h2r,"GSP #it{b}-jets","p");
   }
   ...
   if(isSingleMuon){
     ...
     if(isMu7) la->DrawLatexNDC(0.58,0.5,"9 < #font[52]{p}_{T}^{#mu} (GeV) < 14, |#it{#eta}^{#mu}| < 2.0");
   ...
   if(isC2){
     ...
     la->DrawLatexNDC(0.32,0.8,"Cent. 0-10%");
   }
   else if(isC1){
   ...
   if(isPP){
     if(isSingleMuon){
       if(isMu7){
	 c1->SaveAs("/home/clayton/Analysis/code/bJetMuonTaggingAnalysis/figures/jetPt/SingleMuon/pp/jetPt_SingleMuon_pp_mu7.pdf");
       }
       else if(isMu12){
     ...
     if(isMinBias){
       c1->SaveAs("/home/clayton/Analysis/code/bJetMuonTaggingAnalysis/figures/jetPt");
     }
   }
