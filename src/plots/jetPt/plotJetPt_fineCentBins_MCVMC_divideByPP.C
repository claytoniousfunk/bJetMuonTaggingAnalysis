...
void plotJetPt_fineCentBins_MCVMC_divideByPP(bool isPP = 1,
						 bool isC4 = 0,
						 bool isC3 = 0,
						 bool isC2 = 0,
						 bool isC1 = 0){
  ...
  f1 = TFile::Open("/home/clayton/Analysis/code/bJetMuonTaggingAnalysis/rootFiles/scanningOutput/PYTHIA/final/allTemplates/PYTHIA_DiJet_withGS_mu12_tight_pTmu-14_pThat-15_removeHYDJETjet_jetTrkMaxFilter_vzReweight_jetPtReweight_dRReweight_projectableTemplates_allTemplates.root");
  ...
  if(isPP){
    ...
    f1->GetObject("h_inclRecoJetPt_inclRecoMuonTag_triggerOn_flavor",H2);
  }
  else if(isC4){
    f3->GetObject("h_inclRecoJetPt_inclRecoMuonTag_triggerOn_flavor_C4",H1);
    f1->GetObject("h_inclRecoJetPt_inclRecoMuonTag_triggerOn_flavor",H2);
  }
  else if(isC3){
    f3->GetObject("h_inclRecoJetPt_inclRecoMuonTag_triggerOn_flavor_C3",H1);
    f1->GetObject("h_inclRecoJetPt_inclRecoMuonTag_triggerOn_flavor",H2);
  }
  else if(isC2){
    f3->GetObject("h_inclRecoJetPt_inclRecoMuonTag_triggerOn_flavor_C2",H1);
    f1->GetObject("h_inclRecoJetPt_inclRecoMuonTag_triggerOn_flavor",H2);
  }
  else if(isC1){
    f3->GetObject("h_inclRecoJetPt_inclRecoMuonTag_triggerOn_flavor_C1",H1);
    f1->GetObject("h_inclRecoJetPt_inclRecoMuonTag_triggerOn_flavor",H2);   
  }
  else{};
  ...
  if(isPP){
    ...
    leg->AddEntry(h1r,"PYTHIA+HYDJET 50-90%","p");
    leg->AddEntry(h2r,"PYTHIA","p");
  }
  else if(isC3){
    leg->AddEntry(h1r,"PYTHIA+HYDJET 30-50%","p");
    leg->AddEntry(h2r,"PYTHIA","p");
  }
  else if(isC2){
    leg->AddEntry(h1r,"PYTHIA+HYDJET 10-30%","p");
    leg->AddEntry(h2r,"PYTHIA","p");
  }
  else if(isC1){
    leg->AddEntry(h1r,"PYTHIA+HYDJET 0-10%","p");
    leg->AddEntry(h2r,"PYTHIA","p");
  }  
