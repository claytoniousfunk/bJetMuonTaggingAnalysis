...
void fraction_plot_ppOnly(bool do_mu5 = 1,
			  bool do_mu7 = 0,
			  bool do_mu12 = 0){
  ...
  if(do_mu5){
    ...
    input_PYTHIAHYDJET    = goldenFile_PYTHIAHYDJET_DiJet_mu7_pThat15;
    ...
    input_PYTHIAHYDJET    = goldenFile_PYTHIAHYDJET_DiJet_mu12_pThat15;
  ...
  else if(do_mu7) b_purity_file_string = "../../../rootFiles/bPurityResults/bPurityResults_DATA-1_mu5-0_mu7-1_mu12-0_mergeB-1_mergeC-1_pTrel-0.0-5.0_cMult-1.0_bMult-1.2-JERsmear-1.root";
  else if(do_mu12) b_purity_file_string = "../../../rootFiles/bPurityResults/bPurityResults_DATA-1_mu5-0_mu7-0_mu12-1_mergeB-1_mergeC-1_pTrel-0.0-5.0_cMult-1.0_bMult-1.2-JERsmear-1.root";
  else{};
  ...
  for(int i = 0; i < 4; i++){
    cout << "bAmp1(" << i << ") = " << ba1_R->GetBinContent(i) << endl;
    cout << "bAmp2(" << i << ") = " << ba2_R->GetBinContent(i) << endl;
    cout << "bAmp3(" << i << ") = " << ba3_R->GetBinContent(i) << endl;
  }
  ...
  s1->GetYaxis()->SetRangeUser(0,0.10);
  s1->SetTitle("");
  s1->SetStats(0);
  s1->Draw("e2");
  s2->Draw("e2 same");
  ...
  for(int i = 0; i < 4; i++){
    cout << "r2(" << i << ") = " << r2->GetBinContent(i) << endl;
  }
