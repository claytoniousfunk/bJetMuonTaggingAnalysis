...
void plotJetPhi(bool isPP = 1, bool isC2 = 0, bool isC1 = 0, bool isSingleMuon = 1, bool isMinBias = 0, bool isMu7 = 0, bool isMu12 = 1){
  ...
  if(isC2 || isC1){
	...
	// f2 = TFile::Open(goldenFile_PYTHIAHYDJET_DiJet_mu12_pThat30);
  ...
  if(isPP){
    if(isSingleMuon){
      f1->GetObject("h_inclRecoJetPhi_inclRecoMuonTag_triggerOn",h1);
      f2->GetObject("h_inclRecoJetPhi_inclRecoMuonTag_triggerOn_flavor",H2);
    }
    else if(isMinBias){
      f1->GetObject("h_inclRecoJetPhi",h1);
      f2->GetObject("h_inclRecoJetPhi_flavor",H2);
    }
    else{};
  }
  ...
  if(isC2){
    if(isSingleMuon){
      f1->GetObject("h_inclRecoJetPhi_inclRecoMuonTag_triggerOn_C1",h1);
      f2->GetObject("h_inclRecoJetPhi_inclRecoMuonTag_triggerOn_flavor_C1",H2);
    }
    else if(isMinBias){
      f1->GetObject("h_inclRecoJetPhi_C2",h1);
      f2->GetObject("h_inclRecoJetPhi_flavor_C2",H2);
    }
    else{};
  }
  ...
  if(isC1){
    if(isSingleMuon){
      f1->GetObject("h_inclRecoJetPhi_inclRecoMuonTag_triggerOn_C1",h1);
      //f2->GetObject("h_inclGenJetPhi_inclRecoMuonTag_triggerOn_flavor_C1",H1);
      //f2->GetObject("h_inclRecoJetPhi_inclRecoMuonTag_triggerOn_flavor_C1",H2);
      f2->GetObject("h_inclRecoJetPhi_inclRecoMuonTag_triggerOn_flavor_C1",H2);
    }
    else if(isMinBias){
      f1->GetObject("h_inclRecoJetPhi_C1",h1);
      //f2->GetObject("h_inclGenJetPhi_inclRecoMuonTag_triggerOn_flavor_C1",H1);
      f2->GetObject("h_inclRecoJetPhi_inclRecoMuonTag_triggerOn_flavor_C1",H2);
    }
    else{}
  ...
  p1->cd();
   ...
   //double muRelPtAxis[M] = {-(50.)*pi/50.,-(40.)*pi/50.,-(30.)*pi/50.,-(20.)*pi/50.,-(10.)*pi/50.,-(0.)*pi/50.,10*pi/50.,20.*pi/50.,30.*pi/50.,40.*pi/50.,50.*pi/50.};
  ...
  h1r->GetYaxis()->SetRangeUser(0.08,0.35);
  h1r->Draw();
  h2r->Draw("same");
  ...
  if(isSingleMuon){
    ...
    //la->DrawLatexNDC(0.58,0.5,"#font[52]{p}_{T}^{#mu} > 7 GeV, |#phi^{#mu}| < 2.0");
    if(isMu7) la->DrawLatexNDC(0.58,0.5,"9 < #font[52]{p}_{T}^{#mu} (GeV) < 14, |#it{#eta}^{#mu}| < 2.0");
    if(isMu12) la->DrawLatexNDC(0.58,0.5,"#font[52]{p}_{T}^{#mu} > 14 GeV, |#it{#eta}^{#mu}| < 2.0");
  }
