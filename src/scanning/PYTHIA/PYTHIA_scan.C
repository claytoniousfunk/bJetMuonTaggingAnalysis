...
void PYTHIA_scan(int group = 1){
  ...
  TString input = Form("/eos/cms/store/group/phys_heavyions/cbennett/output_skim_PYTHIA_DiJet_withGS/PYTHIA_DiJet_skim_output_%i.root",group);
  TString output = Form("/eos/cms/store/group/phys_heavyions/cbennett/scanningOutput/output_PYTHIA_DiJet_withGS_mu12_tight_pTmu-14_pThat-30_removeHYDJETjets_leadingXjetDump_jetPtReweight_vzReweight_bJetEnergyShift_newFitMethod/PYTHIA_DiJet_scan_output_%i.root",group);
  ...
  for(int evi = 0; evi < NEvents ; evi++){
	...
	if(evtTriggerDecision){
	  ...
	  h_muptrel_inclRecoMuonTag_triggerOn_flavor[0]->Fill(muPtRel_i,jetFlavorInt,w_jet);
