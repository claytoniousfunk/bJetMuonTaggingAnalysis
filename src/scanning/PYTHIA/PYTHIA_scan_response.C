...
void PYTHIA_scan_response(int group = 1){
  ...
  TString output = Form("/eos/cms/store/group/phys_heavyions/cbennett/scanningOutput/output_PYTHIA_mu12_response_pThat-30_muTaggedJets_doBJetNeutrinoEnergyShift_pTcut-30/PYTHIA_DiJet_scan_output_%i.root",group);
  ...
  for(int evi = 0; evi < NEvents ; evi++){
	...
	if(dr < minDr){ 
      ...
      if(hasRecoJetMatch && hasRecoJetMuon && matchedRecoJetPt > 30) {   // keep only muon-tagged jets
