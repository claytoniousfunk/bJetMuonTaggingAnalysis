...
void PbPb_trigger_scan(int group = 1){
  ...
  TString output = Form("/eos/cms/store/group/phys_heavyions/cbennett/scanningOutput/output_PYTHIAHYDJET_DiJet_triggerEffScan_Jet80HLT_mu12_tight_onlyOneMuonPerEvent/PYTHIAHYDJET_scan_output_%i.root",group);
  ...
  for(int evi = 0; evi < NEvents; evi++){
    ...
    if(fabs(em->vz) > 15.0 || hiBin_shifted > 160 || hiBin_shifted < 0) continue;
	...
	if(hiBin_shifted >= 0 && hiBin_shifted <=20){
	...
	if(hiBin_shifted > 20 && hiBin_shifted <=60){
	...
	if(hiBin_shifted > 60 && hiBin_shifted <=100){
	...
	if(hiBin_shifted > 100 && hiBin_shifted <=160){
