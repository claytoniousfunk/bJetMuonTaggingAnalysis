...
void PYTHIAHYDJET_scan(int group = 1){
  ...
  TString input = Form("/eos/cms/store/group/phys_heavyions/cbennett/skims/dripping-tap/output_skim_PH_DiJet_batch2n/PYTHIAHYDJET_DiJet_skim_output_%i.root",group);
  TString output = Form("/eos/cms/store/group/phys_heavyions/cbennett/scanningOutput/output_PH_DiJet_batch2_withGS_scan_mu12_tight_pTmu-14_pThat-15_hiHFcut_removeHYDJETjet_jetTrkMaxFilter_vzReweight_hiBinReweight_jetPtReweight_fineCentBins_projectableTemplates_allTemplates/PYTHIAHYDJET_scan_output_%i.root",group);
