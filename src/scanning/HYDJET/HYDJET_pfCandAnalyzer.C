...
void HYDJET_pfCandAnalyzer(int group = 1){
  ...
  std::string inputFileList = "../../../fileNames/fileNames_MinBias_Hydjet_Drum5F_2018_5p02TeV_withTracksAndPFCandidates.txt";
  ...
  while(instr>>filename){
	...
	cout << "evt frac: " << evi_frac << "%" << endl;
      ...
      // global event cuts
      // if(em->pthat <= pthatcut) continue; // no pthat cut in HYDJET
	...
	if(triggerIsOn(triggerDecision_mu12,triggerDecision_mu12_Prescl)){
      ...
      int N_generatedPseudoJets = 1; // define how many psuedo jets to create
